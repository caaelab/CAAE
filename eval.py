...
RELEASE_EVAL_PARAMS = {
    "msl": {"alpha": 0.85, "smooth_kernel": 5, "morph_kernel": 17},
    "smap": {"alpha": 0.9, "smooth_kernel": 1, "morph_kernel": 17},
    "smd": {"alpha": 0.1, "smooth_kernel": 25, "morph_kernel": 29},
    "swat": {"alpha": 0.5, "smooth_kernel": 9, "morph_kernel": 51},
    "wadi": {"alpha": 0.85, "smooth_kernel": 17, "morph_kernel": 29},
}
...
def smooth_scores(x: np.ndarray, kernel: int = 5) -> np.ndarray:
...
def evaluate_release_fusion(
    ...
    s = np.clip(s, 0.0, 1.0)
    s = smooth_scores(s, kernel=smooth_kernel)
    s = np.clip(s, 0.0, 1.0)
    s = score_morphology(s, kernel=score_morph_kernel)
    s = np.clip(s, 0.0, 1.0)
...
def evaluate_one_checkpoint(
    ...
    dataset_key = str(config["data"]["dataset_name"]).lower()
    release_eval = RELEASE_EVAL_PARAMS.get(dataset_key)
    if release_eval is None:
        raise ValueError(f"No release evaluation parameters registered for dataset: {dataset_key}")
    release_alpha = float(release_eval["alpha"])
    smooth_kernel = int(release_eval["smooth_kernel"])
    score_morph_kernel = int(release_eval["morph_kernel"])
    ...
    fused = smooth_scores(fused, kernel=smooth_kernel)
    fused = np.clip(fused, 0.0, 1.0)
    fused = score_morphology(fused, kernel=score_morph_kernel)
    fused = np.clip(fused, 0.0, 1.0)
