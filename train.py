...
RELEASE_LAMBDA_REC = 1.0
RELEASE_LAMBDA_CON = 0.02
RELEASE_LAMBDA_ADV = 0.1
...
def train(config_path='./config.yaml'):
    ...
    for loop_idx, current_ts_name in enumerate(ts_name_list):
        ...
        for epoch in range(start_epoch, num_epochs_total):
            ...
            lambda_rec = RELEASE_LAMBDA_REC
            lambda_con = RELEASE_LAMBDA_CON
            lambda_adv = RELEASE_LAMBDA_ADV
            ...
            for batch_idx, batch_pack in enumerate(tqdm(train_loader, desc=f"Epoch {actual_epoch}")):
                ...
                if projected_anomalous is not None:
                ...
                encoded_negative = encoder(projected_anomalous) if projected_anomalous is not None else None
                ...
                if encoded_negative is not None and anomalous_samples is not None:
